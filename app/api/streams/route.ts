...
export async function POST(req: NextRequest) {
  try {
    ...
    if (signature && typeof signature === "string") {
      ...
      // checking if signature is not pre used
      const existingSignature = await prismaClient.stream.findFirst({
        where: {
          signature,
        },
      });
      ...
      if (paymentAmount < 7300000) {
        return NextResponse.json(
          {
            message: "Payment amount is less than 0.000073",
          },
          {
            status: 411,
          }
