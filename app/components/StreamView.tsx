...
export default function StreamView({
  ...
  const arrageStreams = (streams: Video[]) => {
    const paidStreams = streams.filter((x) => x.paid);
    const freeStreams = streams.filter((x) => !x.paid);
    let arrangedStreams = [];
    if (paidStreams.length > 0) {
      arrangedStreams = [
        ...paidStreams.sort((a, b) => (a.upvotes < b.upvotes ? 1 : -1)),
        ...freeStreams.sort((a, b) => (a.upvotes < b.upvotes ? 1 : -1)),
      ];
    } else {
      arrangedStreams = freeStreams.sort((a, b) =>
        a.upvotes < b.upvotes ? 1 : -1
      );
    }
    return arrangedStreams;
  };
  ...
  async function refreshStreams() {
    ...
    setQueue(arrageStreams(json.streams));
  ...
  const getPaymentAndAddToQueue = async () => {
    setLoading(true);
    try {
      const {
          ...
          <div className="col-span-2">
            <div className="max-w-4xl mx-auto p-4 space-y-6 w-full">
              ...
              <div className="space-y-2">
                ...
                <Button
                  disabled={loading}
                  onClick={getPaymentAndAddToQueue}
                  type="submit"
                  //   className="w-full bg-purple-700 hover:bg-purple-800 text-white"
                  className="w-full "
                  variant={"secondary"}
                >
                  {loading ? "Loading..." : "Pay and Play Now"}
                </Button>
              </div>
